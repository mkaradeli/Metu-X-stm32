...
FRESULT sd_recreate_log_file(char* filename, uint16_t log_index, uint16_t* secondary_log_index);
char filename[32] = {0};
...
//						else
						...
						file_open = sd_reopen_log_append(log_index);
						    if (file_open == FR_OK) {
						        ...
						        FSIZE_t aligned = (sz / sizeof(SensorData_t)) * sizeof(SensorData_t);
						        if (aligned != sz) {
						            f_lseek(&Fil, aligned);
...
//						snprintf(RW_Buffer, sizeof(RW_Buffer), "timestamp = %ld\n\r", uwTick);
							FR_Status = f_write(&Fil, &sensorData, sizeof(sensorData), &WWC);
...
//				printf("DISK STATUS = %d\n\r", disk_mounted);
	...
	printf("sd_card task mean time = %d.%d, cpu usage = %d.%d \n\r", (int)sd_card_write_profiler.mean_time, FRACTIONAL(sd_card_write_profiler.mean_time), (int)sd_card_write_profiler.cpu_usage, FRACTIONAL(sd_card_write_profiler.cpu_usage));
//	printf("%lu\n\r", __HAL_TIM_GET_COUNTER(&htim7));
	printf("%d, %p, %d, %d\n\r", disk.is_initialized[0],disk.drv[0],disk.lun[0],disk.nbr);
//	printf("%lu\n\r", (uint32_t)micros());
