...
/* DMA cannot reach DTCM, so the landing zone lives in D2 like the ADC
 * buffers. 32-byte aligned and sized so the cache maintenance below can
 * never touch a neighbouring variable. */
__attribute__((section(".RAM_D2_Section"), aligned(32), used))
